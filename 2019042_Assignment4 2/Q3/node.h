...
class Node {
 ...
 public:
  ...
  void updateTblEntry(string dest_ip)
  {

      int tblsize = mytbl.tbl.size();
      int interfacesize = interfaces.size();

      //Changing ip entries and changing cost to 16
      for(int i=0; i<tblsize;i++)
      {
        string myDestIp = mytbl.tbl[i].dstip;
        if(myDestIp == dest_ip)
        {
          mytbl.tbl[i].cost = 16;
        }
      ...
      {
        string intrfcDest = interfaces[i].first.getConnectedIp();
        if(intrfcDest == dest_ip)
        {
  ...
  void sendMsg(){
    ...
    int interfaceSize = interfaces.size();
    int mytblSize = mytbl.tbl.size();
    for (int i = 0; i < interfaceSize ;++i) {
      ntbl.tbl.clear();
      for(int j=0;j<mytblSize;j++){
        ntbl.tbl.push_back(mytbl.tbl[j]);
      }
      for(int k=0;k<mytblSize;k++){
        string nhop = mytbl.tbl[k].nexthop;
        if(interfaces[i].second->isMyInterface(nhop)){
          ntbl.tbl[k].cost = 16;
        }
      }
      RouteMsg msg;
      string first_ip = interfaces[i].first.getip();
      msg.from = first_ip;
      msg.mytbl = &ntbl;
      string second_ip = interfaces[i].first.getConnectedIp();
      msg.recvip = second_ip;
